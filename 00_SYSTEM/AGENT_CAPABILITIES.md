...
| `call_mcp_tool` (`firecrawl`) | ~24 Tools: `firecrawl_scrape`, `firecrawl_map`, `firecrawl_crawl`, `firecrawl_search`, `firecrawl_monitor_*`, `firecrawl_agent`, etc. | **Live Web Scraping & Research (added 2026-09-03):** Fetch/scrape real URLs including JS-rendered pages and PDFs, enumerate a site's URLs, run web search, or set up a recurring change-monitor. Registered in both your native `agentica` config and the bundled Cline extension's config — dev-time only. **Does not extend to production**: deployed Vercel serverless functions (e.g. `server-lib/cron/ingest/extract.ts`) cannot reach your IDE's MCP tools and must call Firecrawl's own REST API directly instead. Prior note that you have "no web-fetch tool" (see `01_CONTROL/STATE.md`'s sourced-reference-material entry) is now out of date for interactive/dev-time use only. | Credit-metered (Firecrawl API usage) |
