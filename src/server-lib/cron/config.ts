...
export function getCronConfig(): CronConfig {
  ...
  // Strict env var validation — no hardcoded fallbacks
  if (!env("SUPABASE_URL")) {
    throw new Error("SUPABASE_URL environment variable is required for cron config");
  }
  if (!env("SUPABASE_SERVICE_ROLE_KEY")) {
    throw new Error("SUPABASE_SERVICE_ROLE_KEY environment variable is required for cron config");
  }
