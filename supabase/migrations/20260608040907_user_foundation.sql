-- ============================================================
-- Migration: User-Centric Foundation & Arena Overhaul
-- Date: 2026-06-08
-- Description:
--   - user_profiles table (extends auth.users, includes membership_tier)
--   - quiz_sessions table (correct_count, incorrect_count, unattempted_count naming)
--   - question_attempts table (enables "No-Repeat" quiz logic, question_id as text)
--   - saved_insights table (bookmark engine, question_id as text for UUID compat)
--   - Auto-create user_profiles on auth.users INSERT via trigger
--   - get_user_percentile RPC function
--   - Row Level Security policies on all tables (permissive for authenticated)
--   - Explicit GRANT ALL to anon, authenticated, service_role
--   - Indexes for performance
-- ============================================================

-- Drop tables in dependency-safe order (CASCADE handles FK refs)
...
-- ----------------------------
-- 2. quiz_sessions  (correct_count / incorrect_count / unattempted_count naming)
-- ----------------------------
...
-- Permissive INSERT policy for authenticated + anon (fixes "permission denied")
...
-- Permissive SELECT policy for authenticated + anon
...
-- ----------------------------
-- 3. question_attempts (question_id as text for UUID string compatibility)
-- ----------------------------
...
-- ----------------------------
-- 4. saved_insights (question_id as text for UUID string compatibility)
-- ----------------------------
...
-- ----------------------------
-- 6. Explicit GRANT ALL to application roles
-- ----------------------------
