...
## 2. Movement
1. **Request Questions**: Client fetches practice items via `GET /api/training-questions`.
2. **Practice Arena**: Candidate solves questions in Arena with `is_training = true`.
3. **Submit Answers**: Client posts answers with `is_training: true` to `/api/submit-quiz`.
