...
3. **Fetch Answer Key**: Backend pulls `correct_index` directly from database `questions` table.
4. **Compute Scores**: Computes total marks, positive/negative marks, and accuracy percentage.
5. **Update Profile**: Updates `profiles.xp`, `profiles.streak`, and `profiles.last_active_at`.
6. **Persist Session**: Inserts row into `quiz_sessions`.
